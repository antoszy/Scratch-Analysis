...
for scratchList in sampleList:
    ...
    for scratchObject in scratchList:
        ...
        scratchObject.scratch.fricCoeff = convolve(scratchObject.scratch.fricCoeff, ones(10,)/10, mode='same')
        
        #computing max dephts
        sum1 = sum1 + scratchObject.meanFrictCoeffOfScratch()
        maxList.append(scratchObject.meanFrictCoeffOfScratch())
...
for i in range(4):
    ...
    for scratchObject in sampleList[i]:
        if flag == 0:
            flag = 1
            pyplot.plot( scratchObject.scratch.distance, scratchObject.scratch.fricCoeff,  colorList[i], label=labelList[i])
        else:
            pyplot.plot( scratchObject.scratch.distance, scratchObject.scratch.fricCoeff,  colorList[i])
